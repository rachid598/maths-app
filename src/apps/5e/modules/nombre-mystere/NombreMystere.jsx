...
export default function NombreMystere({ onBack }) {
  ...
  if (done) {
    return (
      <div className="theme-5e">
        <ScoreScreen
          score={score}
          total={PROBLEMS}
          onReplay={() => selectLevel(level)}
    ...
    <div className="min-h-screen flex flex-col p-4">
      ...
      <div className="h-1.5 bg-surface rounded-full mb-4 overflow-hidden">
        <div
          ...
          style={{ width: `${((index + (solved ? 1 : 0)) / PROBLEMS) * 100}%` }}
        />
      </div>
      ...
      {riddle && !solved && (
        <button
          ...
          className="mx-auto mt-3 flex items-center gap-1.5 text-sm text-indigo-300 hover:text-indigo-200 transition-colors"
        >
      ...
      {/* Equation reveal on solve */}
      {riddle && solved && (
        <div className="mt-4">
          <EquationReveal equation={riddle.equation} x={riddle.x} visible />
        </div>
      )}
          ...
          <div className="grid grid-cols-3 gap-2">
            {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((n) => (
              <button
                key={n}
                type="button"
                onClick={() => input.length < 4 && setInput(input + String(n))}
                className="py-3 rounded-xl bg-surface-light text-lg font-semibold active:bg-primary/30 transition-colors"
              >
                {n}
              </button>
            ))}
