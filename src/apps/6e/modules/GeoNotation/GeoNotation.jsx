...
const SCORES_KEY = KEYS_6E.custom('gn_scores')
...
export default function GeoNotation({ player, onBadgeCheck }) {
  ...
  const startLevel = useCallback((lv) => {
    ...
    setQi(0); setScore(0); setStreak(0); setSelected(null); setPhase('play')
  }, [])
  ...
  const handleChoice = useCallback((choiceIndex) => {
    ...
    feedbackTimer.current = setTimeout(() => {
      ...
      if (qi + 1 >= QUESTIONS_PER_ROUND) {
        ...
        addHistory({ module: 'GN', level: level.id, score: finalScore, total: QUESTIONS_PER_ROUND })
  ...
  }, [question, selected, qi, score, bestScores, level, playSuccess, playError, playConfetti, onBadgeCheck, addHistory])
    ...
    return (
      <PageTransition><div className="flex flex-col min-h-screen bg-surface dark:bg-gray-900">
        <header className="flex items-center gap-3 px-4 py-3 bg-white/80 dark:bg-gray-800/80 border-b border-primary/10">
          ...
          <div><h1 className="font-extrabold text-primary-dark dark:text-primary-light text-lg">{'\u270F\uFE0F'} Geo-Notation</h1><p className="text-xs text-gray-400">Choisis ton niveau</p></div>
        </header>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-surface dark:bg-gray-900">
      <header className="flex items-center justify-between px-4 py-3 bg-white/80 dark:bg-gray-800/80 border-b border-primary/10">
        ...
        <span className="text-sm font-bold text-primary-dark dark:text-primary-light">{'\u270F\uFE0F'} {level.label} — {level.title}</span>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 gap-5">
        ...
        {question?.figure && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-md w-full max-w-sm">
            <FigureSVG kind={question.figure.kind} points={question.figure.points} />
          </div>
        )}
